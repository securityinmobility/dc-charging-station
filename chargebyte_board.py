...
class ChargebyteBoard:
    ...
    def check_response(self, service_id:int, response:bytearray)->None:
        if response[0] != 0x02:
            raise ValueError('beginning of message was not 0x02')
        if response[3] != service_id + 0x80:
            raise ValueError('this response does not corresponds to the service that was requested')
        if self.check_block_sum(response[:-1]) != response[-1]:
            raise ValueError('Something went wrong: the check block is wrong!')
    ...
    def cyclic_process_data(self, interval:int):
        interval = 2
        #TO DO: this one requires the use of threads to receive the data each time on the interval
        return
